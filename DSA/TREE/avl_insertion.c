...
long arrayManipulation(int n, int queries_rows, int queries_columns, int** queries) {
    ...
    while(m<queries_rows){
        printf("<%d>---...%ld + %d...\n",queries[m][0]-1,arr[queries[m][0]-1],queries[m][2]);
        arr[queries[m][0]-1]=arr[queries[m][0]-1]+queries[m][2];
        printf("<%d>---...%ld - %d...\n",queries[m][1],arr[queries[m][1]],queries[m][2]);
        arr[queries[m][1]]=arr[queries[m][1]]-queries[m][2];
        m++;
    }
    long max=0;
    for(i=1;i<n;i++){
        printf("<%ld>--------->>>>..%ld + %ld ------",i,arr[i],arr[i-1]);
        arr[i]=arr[i]+arr[i-1];
        printf("---%ld--\n",arr[i]);
        if(arr[i]>max)
            max=arr[i];
